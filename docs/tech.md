...
5. **System Setup:** The user then runs the newly downloaded binary using `./grew setup` (usually via `sudo`). The `setup` command initializes the system prefix (`/opt/homegrew` on Apple Silicon, `/usr/local/homegrew` on Intel).
