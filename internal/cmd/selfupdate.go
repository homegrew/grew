...
func tryPatchUpdate(exePath string, rel *release.Release) error {
	...
	// First, try to fetch the specific .sha256 / .sha512 files generated by the patcher tool.
	sha256URL, err256 := release.FindAssetURL(rel, patchName+".sha256")
	sha512URL, err512 := release.FindAssetURL(rel, patchName+".sha512")
	...
	if err256 == nil || err512 == nil {
		...
		// Fallback: Verify patch hash against the monolithic checksums.txt
		checksumURL, err := release.FindAssetURL(rel, "checksums.txt")
		if err != nil {
			return err
		}
