...
func init() {
	Command.Flags().BoolVar(&auditStrict, "strict", false, "Treat warnings as errors (exit non-zero on warnings)")
	Command.Flags().BoolVar(&auditCask, "cask", false, "Audit cask definitions instead of formulas")
	Command.Flags().BoolVar(&auditOnline, "online", false, "Include checks that require installed packages (verifies snapshot integrity)")
}
