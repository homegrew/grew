...
func init() {
	Command.Flags().BoolVar(&linkageTest, "test", false, "Only report broken dependencies (exit 1 if any)")
	Command.Flags().BoolVar(&linkageStrict, "strict", false, "Also check for undeclared and unused dependencies")
	Command.Flags().BoolVar(&linkageReverse, "reverse", false, "Show formulas that link against this formula's libraries")
	Command.Flags().BoolVar(&linkageCached, "cached", false, "Use cached linkage results if available")
	Command.Flags().BoolVarP(&linkageQuiet, "quiet", "q", false, "Only output broken dependencies")
}
